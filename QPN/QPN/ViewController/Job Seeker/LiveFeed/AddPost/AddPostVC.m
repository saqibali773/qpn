...
- (void)viewDidLoad {
    ...
    self.userName.text = [NSString stringWithFormat:@"%@. %@",SHARED_MANAGER.qpnUser.salutation,SHARED_MANAGER.qpnUser.first_name];
    ...
    if (self.isEditing)
    {
        ...
        self.postText.text = _postObj.text;
       // float height = [self textViewHeightForAttributedText:_postObj.text andWidth:screenWidth- 62 heightWithoutText:32.0];
       // self.postTextHeight.constant = height;

        self.attachImageBtn.hidden = YES;
    }
}
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    {
        self.postTextHeight.constant = 109.0;
    }
    else
    {
        self.postTextHeight.constant = 109.0;
...
- (void)uploadPost:(NSString *)imageUrl{
    ...
    if(!self.isEditing)
    {
        ...
        [SHARED_MANAGER showWorkingSpinner:self.view text:@"Updating Post.."];
        NSDictionary *parameters = @{ @"post": @{ @"text" : self.postText.text} };
        [AUTH_MAN editPost:self.postObj.post_id params:parameters parameterEncoding:JSON completionHanlder:^(id response, NSError *error) {
            [SHARED_MANAGER hideWorkingSpinner:self.view];
            if(response) {
